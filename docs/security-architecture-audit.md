...
|----|------|--------|--------|
| **P1** | CSP + `next/image` hosts aligned, env-driven Supabase/R2 | **Done** | `lib/constants/security-hosts.ts` builds CSP (`buildContentSecurityPolicy`) and `getImageRemotePatterns()`; consumed by `middleware.ts` and `next.config.ts`. |
